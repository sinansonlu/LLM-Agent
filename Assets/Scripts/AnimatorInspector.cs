...
public class AnimatorInspector : MonoBehaviour
{
    ...
    void Start()
    {
        Quaternion tmpQ = anim.gameObject.transform.rotation;
        ...
        {
            if (!animClipDict.ContainsKey(animClips[i].name))
            {
            ...
            {
                Debug.Log(animClips[i].name + " is already added in AnimatorInspector.");
            }
        }
        ...
        anim.gameObject.transform.rotation = tmpQ;
